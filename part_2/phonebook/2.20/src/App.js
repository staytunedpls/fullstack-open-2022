    ...
    if (persons.map((person) => person.name).includes(newName)) {
      ...
      if (
        ...
        personService
          ...
          .catch((error) => {
            console.log("Could not change", newName, "got error:", error);
            setPersons(
              persons.filter((person) => person.id !== existingPersonId)
            );
            setNotificationMessage(`Could not change entry ${newName}, \
            likely had already been removed from the server`);
            setNotificationType("error");
            setTimeout(() => setNotificationMessage(null), 5000);
          });
      } else {
      ...
      personService
        ...
        .catch((error) => {
          ...
          setNotificationMessage(`Could not add new person ${newPerson.name}, \
        got error: ${error}`);
          setNotificationType("error");
        });
    }
